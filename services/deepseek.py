...
class DeepSeekService:
    ...
    async def chat(self, messages: list, max_tokens: int = 1500) -> str:
        """
        Generic multi-turn chat. Used by:
          - App builder (agentic code loop)
          - Chart chat (follow-up questions on screenshots)
    ...
    async def analyze_ohlc(
        ...
        Called by webhooks/ohlc.py (MT4/MT5/cTrader bots).
        Returns: {signal, pattern, reason, confidence, levels}
        """
        import json, re

        candle_table = "\n".join(
            f"  [{i+1}] O={c['o']:.5f}  H={c['h']:.5f}  L={c['l']:.5f}  C={c['c']:.5f}"
            for i, c in enumerate(candles[-5:])
        )
        pattern_summary = (
            "\n".join(
                f"  - {p['name'].replace('_',' ').title()} "
                f"({'bullish' if p['bullish'] else 'bearish'}, conf={p['confidence']:.0%})"
                for p in detected_patterns
            ) or "  None detected"
        )
        rule_summary = (
            "\n".join(f"  - {r.get('name','?')}: enabled={r.get('enabled',True)}"
                       for r in personal_rules[:5])
            or "  Using system defaults"
        )
        indicator_str = "  " + ",  ".join(
            f"{k}={v:.4f}" for k, v in (indicators or {}).items()
        ) or "  None provided"

        prompt = (
            f"Analyse this chart data and return a trading signal.\n\n"
            f"Symbol: {symbol}  |  Timeframe: {timeframe}\n\n"
            f"Last 5 candles (oldest→newest):\n{candle_table}\n\n"
            f"Indicators:\n{indicator_str}\n\n"
            f"Detected patterns:\n{pattern_summary}\n\n"
            f"User rules:\n{rule_summary}\n\n"
            f"Respond ONLY with valid JSON (no markdown):\n"
            f'{{"signal":"BUY|SELL|NEUTRAL","pattern":"name","reason":"2-3 sentences",'
            f'"confidence":0.0,"levels":[{{"type":"support|resistance","price":0.0}}]}}'
        )

        messages = [
            {"role": "system", "content": "You are an expert technical analyst. Return valid JSON only."},
        ...
        try:
            ...
            return {
                "signal": "NEUTRAL", "pattern": "",
                "reason": raw[:300], "confidence": 0.0, "levels": [],
            }
