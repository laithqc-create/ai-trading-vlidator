...
class PlanTier(str, PyEnum):
    ...
    PRODUCT1 = "product1"   # Indicator Validator $19
    PRODUCT2 = "product2"   # EA Analyzer $49
    PRODUCT3 = "product3"   # Live Analysis $29
...
class User(Base):
    ...
    def has_product_access(self, product_num: int) -> bool:
        """Unified access check used by all route handlers."""
        if self.is_trial_active():
            return True
        if self.plan == PlanTier.PRO:
            return True
        product_map = {1: PlanTier.PRODUCT1, 2: PlanTier.PRODUCT2, 3: PlanTier.PRODUCT3}
        return self.plan == product_map.get(product_num)
