...
celery_app.conf.beat_schedule = {
    ...
    # Expire free trials nightly at 02:00 UTC
    "expire-trials": {
        "task": "tasks.expire_trials_task",
...
@celery_app.task(name="tasks.expire_trials_task")
def expire_trials_task():
    """
    Downgrade users whose 14-day trial has expired back to FREE.
    ...
    async def _run():
        ...
        async with get_db_context() as db:
            svc = UserService(db)
            count = await svc.expire_trials()
            await db.commit()
            logger.info(f"Trial expiry: {count} users downgraded to FREE")
        return count
